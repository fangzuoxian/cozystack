...
- **Air-gapped tenant workers (Phase 1 Talos rollover)**: tenant worker bootstrap moves from Ubuntu containerDisk + kubeadm to a Talos OS disk image fetched over HTTP by CDI and a `.../installer/...` installer reference. Both sources are now overridable — set `talos.imageFactoryURL` (default `https://factory.talos.dev`) to a self-hosted Image Factory, a caching mirror, or an internal HTTP file server, and `talos.installerRepository` (default `factory.talos.dev/installer`) to a mirrored registry. The Helm-rendered `*-patch-containerd` Secret that previously plumbed the platform-wide `registries.mirrors` config to tenant workers (via the kubeadm template's containerd certs.d mount) has no consumer in the Talos machineconfig and was removed in this release. **In-guest container-image pulls** are routable again: `talos.registryMirrors` maps an upstream registry host to `{ endpoints: [ ... ] }` and renders into the worker machineconfig's `machine.registries.mirrors`, so a strict-egress tenant can mirror everything its nodes pull from a registry: the Talos installer, the kubelet image, and the images its own pods pull. The worker OS disk image is not one of them. CDI streams that in as a raw artifact over plain HTTP, outside any registry, so `talos.imageFactoryURL` is the knob for it. Two limits are worth knowing before relying on it. Talos falls back to the upstream registry for a host unless that host also sets `skipFallback: true`, so a mirror on its own routes pulls without enforcing an air gap. And only `machine.registries.mirrors` is passed through — `machine.registries.config` is not, so a mirror served over HTTPS with a private CA, or one requiring credentials, is not configurable this way; nor does the platform-wide `registries` value reach tenant workers any more. Setting the field on a running cluster also does not reconfigure the workers it already has: the value reaches a node through the machine config it boots with, so existing Machines keep pulling as before until they are replaced, and only nodes created after the change use the mirror.
