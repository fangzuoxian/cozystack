...
| `tracingCollector.backend`                   | Export target. "local": the tenant's own tracingStorages backend in this namespace. "central": the shared VictoriaTraces in tenant-root (shared-central topology) — no local backend is deployed, spans are stamped with the tenant name, and the cross-boundary hop uses the collector-scoped Cilium egress rule. | `string`   | `local` |
